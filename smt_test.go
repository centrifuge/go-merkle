...
func TestCacheWithLessThanHalveLeaves(t *testing.T) {
	hash := hashFunc
	items := testHashes[:3]
	hashCount := 0
	decoratedHash := NewHashCountDecorator(hash, &hashCount)
...
func TestCacheWithMoreThanHalveEmptyLeaves(t *testing.T) {
	hash := hashFunc
	items := testHashes[:9]
	hashCount := 0
	decoratedHash := NewHashCountDecorator(hash, &hashCount)
	...
	err := tree.Generate(items, 16)
	assert.Nil(t, err)
	assert.Equal(t, 6+4+2+1, hashCount)

	rootHash, err := tree.RootHash()
	//expectedRoot is generated by TestExample_complete(...) defined in merkle_test.go
	expectedRoot := []byte{152, 2, 216, 141, 25, 184, 18, 213, 247, 132, 128, 128, 213, 9, 64, 121}
	assert.Equal(t, expectedRoot, rootHash)
	assert.Equal(t, 9+5+3+2+1, totalHashes(tree.fullNodes))
}
...
func TestGetMerkleProofs2(t *testing.T) {
	hash := hashFunc
	items := testHashes[:5]
	...
	//proof of []byte("alpha4")
	proof, err := tree.GetMerkleProof(4)

	sibleHash := emptyHash
	proofNode := ProofNode{Left: false, Hash: sibleHash}
	expectedProof := []ProofNode{proofNode}

	sibleHash = hash2Value(emptyHash, emptyHash, hash)
	proofNode = ProofNode{Left: false, Hash: sibleHash}
	expectedProof = append(expectedProof, proofNode)

	tmp1Hash := hash2Value(testHashes[0], testHashes[1], hash)
	tmp2Hash := hash2Value(testHashes[2], testHashes[3], hash)
	sibleHash = hash2Value(tmp1Hash, tmp2Hash, hash)
	proofNode = ProofNode{Left: true, Hash: sibleHash}
	expectedProof = append(expectedProof, proofNode)
